...
1. want와 number로 user가 원하는 것을 딕셔너리로 만든다.
2. discount를 10개씩 끊어서 Counter로 개수가 몇개인지 구한다.
   1. 그것과 user가 원하는것이랑 일치하면 answer에 1을 더한다.
   2. 여기서 dict는 ==을 사용하면 일치하는지 확인할 수 있다.

```python
from collections import Counter
def solution(want, number, discount):
    answer = 0
    user = {obj : cnt for obj, cnt in zip(want, number)}
    for i in range(len(discount) - 9):
        if user == Counter(discount[i:i+10]):
            answer +=1
    return answer
```
