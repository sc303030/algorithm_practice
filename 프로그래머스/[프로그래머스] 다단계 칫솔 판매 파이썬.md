...
- 오늘도 좋은 풀이를 보면서 해석하였다.
  - https://www.youtube.com/watch?v=rXF0-4ejf_M
- `__init__`같은 경우는 `dataclass`를 사용하였다.
- `enroll`과 `referral`의 길이가 같기에 `zip`을 사용해서 한 번에 활용하였고, `seller`과 `amount`도 똑같이 사용하였다.
- 이런 부모 타고 타고 올라가는 문제를 봤었는데 앞으로 이렇게 사용해보아야 겠다.
- 가장 풀이가 깔끔하고 활용하기 쉬워보인다.
...
class Node:
    ...
    def distribute(self, money:int):
        distributed_money = money // 10
        self.val += (money - distributed_money)
        if distributed_money > 0 and self.parent:
            self.parent.distribute(distributed_money)
...
def solution(enroll, referral, seller, amount):
    ...
    for child, parent in zip(enroll, referral):
        dic[child] = Node(dic[parent])
    for s, a in zip(seller, amount):
        dic[s].distribute(a * 100)
    for name in enroll:
