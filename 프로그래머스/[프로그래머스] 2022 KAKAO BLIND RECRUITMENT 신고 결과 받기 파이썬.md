# [프로그래머스] 2022 KAKAO BLIND RECRUITMENT 신고 결과 받기 파이썬
...
- 누구에게 신고 받았고, 신고 했는지 알기 위해 dict를 만들었다.
...
- 신고 내역을 받아서 신고한 사람과 신고 받은 사람 정보를 업데이트 한다.
  - 신고 받은 사람 dict에서 신고한 사람이 리스트에 없다면 추가하고 신고 횟수를 1 증가시킨다.
  - 신고 내역 dict에서 내가 신고한 유저를 업데이트 한다.
  - 모든 내역은 set으로 관리해서 중복을 제거한다.
...
- answer를 유저 수만큼 0으로 만들어 주었다.
- 얼마나 신고 받았는지 저장하기 위해 dict도 만든다.
- set으로 신고 내역의 중복을 제거하였다.
  - 나도 이렇게 신고내역 자체를 set 하였다면 코드가 길게 복잡해지지 않았을 것이다.
- 신고 받은 사람에게 1씩 증가시킨다.
- 만약 신고 내역이 k번 이상이면 해당 유저를 신고했던 사람에게 1씩 증가시킨다.